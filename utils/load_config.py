...
class ConfigLoader:
    ...
    def _parse_accounts(self, filename: str, proxies: List[Proxy], redirect_mode: bool = False) -> Generator[Account, None, None]:
        try:
    ...
    def load(self) -> Config:
        try:
            ...
            reg_accounts = list(self._parse_accounts("register.txt", proxies, redirect_mode=params["redirect_settings"]["enabled"]))
            farm_accounts = list(self._parse_accounts("farm.txt", proxies))
            reverify_accounts = list(self._parse_accounts("reverify.txt", proxies))
